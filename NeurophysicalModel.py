...
def NeurophysicalModel(velocity_1, velocity_2, velocity_3, type_surface, time):
    ...
    global_coord_delta = np.array([-vxvywz[0] * time * math.sin(angle)
                                   - vxvywz[1] * time * math.cos(angle),
                                   -vxvywz[0] * time * math.cos(angle)
                                   + vxvywz[1] * time * math.sin(angle)])

    return global_coord_delta[0],  global_coord_delta[1], angle


print(NeurophysicalModel(0.47686005, 0.11331177, 1.4467163, 2, 0.5))
